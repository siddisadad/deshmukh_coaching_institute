        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {faculty.map((person, index) => (
            <Reveal key={person.slug} delay={index * 70}>
              <article className="overflow-hidden rounded-[1.7rem] bg-white shadow-soft ring-1 ring-line">
                ...
                <div className="p-5">
                  <p className="text-xs font-bold uppercase tracking-[0.16em] text-royal">{person.subject}</p>
                  <h3 className="mt-1 text-lg font-extrabold text-navy">{person.name}</h3>
